...
import { OVVHC, OVHC, OHC, OMC, OLC, OVLC, OVVLC } from './GenerationChances';


export class TerrainRules {
    private rules: TerrainRule[] = [];
...
const deepWaterRule: TerrainRule = {
    terrainName: TerrainName.T_DEEP_WATER,
    allowedObjects: [ObjectType.water, ObjectType.animal],
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OHC) return ObjectType.water;
        if (roll < OMC) return ObjectType.animal;
        return null;
    },
};

// Правило для мелкой воды
const shallowWaterRule: TerrainRule = {
    terrainName: TerrainName.T_SHALLOW_WATER,
    allowedObjects: [ObjectType.water, ObjectType.animal, ObjectType.plant],
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OVHC) return ObjectType.water;
        if (roll < OMC) return ObjectType.animal;
        if (roll < OLC) return ObjectType.plant;
        return null;
    },
};

// Правило для песка
const sandRule: TerrainRule = {
    terrainName: TerrainName.T_SAND,
    allowedObjects: [ObjectType.plant, ObjectType.rock, ObjectType.animal, ObjectType.structure],
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OMC) return ObjectType.plant;
        if (roll < OLC) return ObjectType.rock;
        if (roll < OVLC) return ObjectType.animal;
        if (roll < OVLC) return ObjectType.structure;
        return null;
    },
};

// Правило для равнин
const plainsRule: TerrainRule = {
    ...
    generateObject: (biome, terrain) => {
        ...
        if (biome.name === BiomeType.B_TUNDRA) {
            if (roll < OHC) return ObjectType.plant;
            if (roll < OMC) return ObjectType.rock;
            if (roll < OLC) return ObjectType.tree; 
            if (roll < OVLC) return ObjectType.animal;
            if (roll < OVLC) return ObjectType.structure;
        }
        if (biome.name === BiomeType.B_FOREST) {
            if (roll < OMC) return ObjectType.tree;
            if (roll < OLC) return ObjectType.plant;
            if (roll < OVLC) return ObjectType.rock;
            if (roll < OVLC) return ObjectType.animal;
            if (roll < OVLC) return ObjectType.structure;
        }
        ...
        if (roll < OVVLC) return ObjectType.artifact;
        return null;
    },
};

// Правило для леса
const forestRule: TerrainRule = {
    ...
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OHC) return ObjectType.tree;
        if (roll < OMC) return ObjectType.plant;
        if (roll < OLC) return ObjectType.animal;
        if (roll < OVLC) return ObjectType.rock;
        if (roll < OVLC) return ObjectType.artifact;
        return null;
    },
};
// Правило для холмов
const hillsRule: TerrainRule = {
...
const rockRule: TerrainRule = {
    ...
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OHC) return ObjectType.rock;
        if (roll < OMC) return ObjectType.rock;
        if (roll < OVLC) return ObjectType.plant;
        if (roll < OVLC) return ObjectType.animal;
        if (roll < OVVLC) return ObjectType.artifact;
        return null;
    },
...
// Правило для пещеры
const caveRule: TerrainRule = {
    terrainName: TerrainName.T_CAVE,
    allowedObjects: [ObjectType.ore, ObjectType.rock, ObjectType.animal, ObjectType.artifact],
    generateObject: (biome, terrain) => {
        const roll = Math.random();
        if (roll < OHC) return ObjectType.rock;
        if (roll < OMC) return ObjectType.ore;
        if (roll < OMC) return ObjectType.animal;
        if (roll < OVVLC) return ObjectType.artifact;
        return null;
    },
