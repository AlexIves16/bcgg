...
import { ObjectRegistryType } from './interfaces';

// Реестр объектов
export const ObjectRegistry: ObjectRegistryType = {
  // Деревья
  [ObjectType.tree]: {
...
export function getObjectById(objectId: GameObjectType): any | null {
  for (const objectType in ObjectRegistry) {
    if (ObjectRegistry[objectType as ObjectType][objectId as GameObjectType]) {
      return ObjectRegistry[objectType as ObjectType][objectId as GameObjectType];
    }
  }
  return null;
} 
