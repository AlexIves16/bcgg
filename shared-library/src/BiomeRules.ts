...
import { TVVHC, TVHC, THC, TMC, TLC, TVLC, TVVLC } from './GenerationChances';

// Класс для управления правилами генерации террейнов в биомах
export class BiomeChanceRules {
    private rules: BiomeTerrainRule[] = [];
    ...
    getTerrainFromBiome(biome: BiomeType, terrains: TerrainName[]): TerrainName | null {
        ...
        if (!terrainChances) return null
        // Фильтруем террейны, оставляя только те, что есть в biome.terrainTypes
        const filteredTerrains = terrainChances.terrains.filter(
            (terrainChance) => terrains.includes(terrainChance.terrain)
        );
